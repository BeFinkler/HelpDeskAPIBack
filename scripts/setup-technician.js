...
try {
  ...
  if (!nome || nome.length < 2 || nome.length > 100 || !email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254 || !senha || [...senha].length < 8 || Buffer.byteLength(senha) > 72) {
    throw new Error('Preencha SETUP_TECHNICIAN_NAME, SETUP_TECHNICIAN_EMAIL e SETUP_TECHNICIAN_PASSWORD com dados válidos.');
  }
  pool = createPool(config.db);
  const users = createUsuarioModel(pool);
  if (await users.findByEmail(email)) throw new Error('E-mail já cadastrado. Nenhuma conta ou senha foi alterada.');
  await users.create({ nome, email, senha_hash: await bcrypt.hash(senha, config.bcryptRounds), perfil: 'tecnico' });
