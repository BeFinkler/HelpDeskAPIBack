...
export function ensureEditable(user, ticket, version) {
  ...
  if (ticket.status !== 'ABERTO') throw conflict('A edição é permitida somente enquanto o chamado está aberto.');
}
...
export function ensureTransition(user, ticket, input) {
  ...
  if (input.status === 'EM_ATENDIMENTO') {
    if (ticket.status !== 'ABERTO' || ticket.tecnico_id !== null) throw conflict('Este chamado não está mais disponível para assumir.');
  } else if (input.status === 'CONCLUIDO') {
    if (ticket.status !== 'EM_ATENDIMENTO') throw conflict('Somente chamados em atendimento podem ser concluídos.');
  ...
  } else { throw conflict('Transição de status inválida.'); }
}

export function ensureCanComment(user, ticket) {
  ensureVisible(user, ticket);
  if (ticket.status === 'CONCLUIDO') throw conflict('Chamados concluídos não recebem novos comentários.');
