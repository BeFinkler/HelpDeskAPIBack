...
export function readEnv(env = process.env) {
  ...
  if (!/^([1-9]\d*)(s|m|h)$/.test(expiresIn))
    throw new Error('JWT_EXPIRES_IN inválido. Use, por exemplo, 1h.');
  ...
  if (!['true', 'false'].includes(env.DB_SSL ?? 'false'))
  ...
  if (sslEnabled) {
    ...
    if (!ca.includes('-----BEGIN CERTIFICATE-----'))
      throw new Error('Certificado DB_SSL_CA_BASE64 inválido.');
  }
  return {
    mode,
    production,
    ...
    jwt: {
      secret,
      expiresIn: ttl,
      issuer: required(env, 'JWT_ISSUER'),
      audience: required(env, 'JWT_AUDIENCE'),
    },
    db: {
