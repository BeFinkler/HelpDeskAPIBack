...
export function createApp({ pool, config, logger = console, authLimitMax = 30 }) {
  ...
  app.get('/', (_req, res) =>
    res.json({ data: { name: 'HelpDesk API', version: '1.0.0', docs: '/api-docs' } }),
  );
  ...
  app.use('/api/v1', (req, res, next) => {
    ...
    if (['POST', 'PATCH'].includes(req.method)) {
      if (!req.is('application/json'))
        return next(
          new AppError(415, 'UNSUPPORTED_MEDIA_TYPE', 'Envie Content-Type: application/json.'),
        );
      if (!req.body || typeof req.body !== 'object' || Array.isArray(req.body))
        return next(new AppError(400, 'INVALID_JSON', 'Envie um objeto JSON.'));
    }
    next();
  });
  ...
  app.use(
    '/api/v1/auth',
    authRoutes(createAuthController(createAuthService(users, config)), guard, limiter),
  );
  app.use(
    '/api/v1/chamados',
    chamadoRoutes(createChamadoController(tickets), createComentarioController(tickets), guard),
  );
